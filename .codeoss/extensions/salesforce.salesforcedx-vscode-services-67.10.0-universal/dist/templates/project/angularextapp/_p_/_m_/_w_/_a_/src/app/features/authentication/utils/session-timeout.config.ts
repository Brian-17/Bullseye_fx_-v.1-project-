...
/**
 * Configuration constants for session timeout monitoring. Direct port of the
 * React `sessionTimeoutConfig.ts` — values are byte-for-byte identical so the
 * polling cadence, retry backoff, and warning window match exactly.
 */
...
/** sessionStorage keys used by the session validator. */

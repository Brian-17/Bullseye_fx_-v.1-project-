...
exports.deactivate = exports.writeToDebuggerOutputWindow = exports.retrieveLineBreakpointInfo = exports.activateEffect = exports.activate = exports.getDebuggerType = exports.updateLastOpened = exports.VSCodeWindowTypeEnum = void 0;
...
const registerCommands = Effect.fn('ApexReplayDebugger.registerCommands')(function* (extensionContext) {
    const api = yield* (yield* effect_ext_utils_1.ExtensionProviderService).getServicesApi;
    const registerCommand = api.services.registerCommandWithRuntime((0, runtime_1.getRuntime)());
    const dialogStartingPathUri = yield* (0, getDialogStartingPath_1.getDialogStartingPath)(extensionContext);
    const promptForLogCmd = vscode.commands.registerCommand('extension.replay-debugger.getLogFileName', async () => {
        ...
        if (fileUris?.length === 1) {
            (0, exports.updateLastOpened)(extensionContext, fileUris[0].fsPath);
    ...
    const launchFromLogFileCmd = vscode.commands.registerCommand('sf.launch.replay.debugger.logfile', async (editorUri) => {
        ...
        if (resolved) {
            (0, exports.updateLastOpened)(extensionContext, resolved.fsPath);
        }
    ...
    const launchFromLastLogFileCmd = vscode.commands.registerCommand('sf.launch.replay.debugger.last.logfile', async () => {
        const lastOpenedLog = extensionContext.workspaceState.get(debuggerConstants_1.LAST_OPENED_LOG_KEY);
    ...
    yield* registerCommand('sf.anon.apex.debug.delegate', anonApexDebug_1.anonApexDebugCommand);
    ...
    return vscode.Disposable.from(promptForLogCmd, launchFromLogFileCmd, launchFromLogFilePathCmd, launchFromLastLogFileCmd, sfCreateCheckpointsCmd, sfToggleCheckpointCmd, launchApexReplayDebuggerWithCurrentFileCmd);
});
...
const activate = async (extensionContext) => {
    (0, extensionProvider_1.setAllServicesLayer)((0, effect_ext_utils_1.buildAllServicesLayer)(extensionContext, messages_1.nls.localize('channel_name')));
    await (0, runtime_1.getRuntime)().runPromise((0, exports.activateEffect)(extensionContext));
};
exports.activate = activate;
exports.activateEffect = Effect.fn('activation:salesforcedx-vscode-apex-replay-debugger')(function* (extensionContext) {
    ...
    const debuggerChannel = yield* channels_1.getDebuggerOutputChannel;
    ...
    extensionContext.subscriptions.push(debuggerChannel, commands, debugHandlers, debugConfigProvider, checkpointsView, breakpointsSub, debugTests, debugTest);
    // Telemetry
    yield* Effect.promise(() => salesforcedx_utils_vscode_1.TelemetryService.getInstance().initializeService(extensionContext));
});
...
const writeToDebuggerOutputWindow = (output, showVSCodeWindow, vsCodeWindowType) => {
    (0, channels_1.appendAndShowChannelOutput)(output);

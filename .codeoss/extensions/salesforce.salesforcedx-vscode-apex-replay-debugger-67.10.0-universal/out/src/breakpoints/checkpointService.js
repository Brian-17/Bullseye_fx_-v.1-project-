...
const SubscriptionRef = require("effect/SubscriptionRef");
...
const getConnection = async () => {
    try {
        return await (0, runtime_1.getRuntime)().runPromise(Effect.gen(function* () {
        ...
        const errorMessage = `${messages_1.nls.localize('unable_to_retrieve_org_info')} : ${(0, Predicate_1.isError)(error) ? error.message : String(error)}`;
...
const clearExistingCheckpoints = async () => {
    try {
        const userId = await (0, runtime_1.getRuntime)().runPromise(Effect.gen(function* () {
            const api = yield* (yield* effect_ext_utils_1.ExtensionProviderService).getServicesApi;
...
const processBreakpointChangedForCheckpoints = (breakpointsChangedEvent) => {
    ...
    Effect.sync(() => {
        ...
        sourceBreakpoints.filter(isCheckpoint).map(bp => {
            ...
            const uri = (0, effect_ext_utils_1.code2ProtocolConverter)(bp.location.uri);
...
const sfCreateCheckpoints = async () => {
    ...
    if (!exports.checkpointService.hasOneOrMoreActiveCheckpoints()) {
...
const sfToggleCheckpoint = () => {
    ...
    if (uri && (0, Predicate_1.isNotUndefined)(lineNumber)) {

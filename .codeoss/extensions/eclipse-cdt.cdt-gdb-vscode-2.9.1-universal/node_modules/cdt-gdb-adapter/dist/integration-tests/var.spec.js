...
describe('Variables Test Suite', function () {
    ...
    it('can read and set struct variables in a program', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            (0, chai_1.expect)(children.body.variables.length, 'There is a different number of child variables than expected').to.equal(4);
    ...
    it('can read and set nested struct variables in a program', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let res = yield dc.evaluateRequest({
                context: 'variables',
                expression: 'r.z',
                frameId: scope.frame.id,
            });
            (0, chai_1.expect)(res.body.result).eq('{\n  "a": 3,\n  "b": 4\n}');
            // Evaluate the child structure foo.baz of r
            res = yield dc.evaluateRequest({
                context: 'variables',
                expression: 'r.aa',
                frameId: scope.frame.id,
            });
    ...
    it('can read and set array elements in a program', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // step the program and see that the values were passed to the program and evaluated.
            yield dc.next({ threadId: scope.thread.id }, { path: varsSrc, line: lineTags['After array init'] + 1 });
            scope = yield (0, utils_1.getScopes)(dc);
            (0, chai_1.expect)(scope.scopes.body.scopes.length, 'Unexpected number of scopes returned').to.equal(2);
            vr = scope.scopes.body.scopes[0].variablesReference;
            vars = yield dc.variablesRequest({ variablesReference: vr });
    ...
    it('can evaluate char array elements (string)', function () {
        return __awaiter(this, void 0, void 0, function* () {
            // skip ahead to array initialization
            const br = yield dc.setBreakpointsRequest({
                source: { path: varsSrc },
            ...
            // step the program and see that the values were passed to the program and evaluated.
            yield dc.next({ threadId: scope.thread.id }, { path: varsSrc, line: lineTags['char string setup'] + 1 });
            scope = yield (0, utils_1.getScopes)(dc);
            (0, chai_1.expect)(scope.scopes.body.scopes.length, 'Unexpected number of scopes returned').to.equal(2);
            // assert we can see the array and its elements
    ...
    it('can evaluate anonymous and deeply nested structs and unions', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const vars = yield dc.variablesRequest({ variablesReference: vr });
            (0, chai_1.expect)(vars.body.variables.length, 'There is a different number of variables than expected').to.equal(numVars);
            (0, utils_1.verifyVariable)(vars.body.variables[11], 'n', 'struct nest', '{...}', {
                hasChildren: true,
            });
            function expandRecursively(variablesReference) {
                return __awaiter(this, void 0, void 0, function* () {
                    if (variablesReference <= 0)
                        return undefined;
                    const vars = (yield dc.variablesRequest({ variablesReference }))
                        .body.variables;
                    const exp = yield Promise.all(vars.map((v) => expandRecursively(v.variablesReference)));
                    return vars.map((v, i) => {
                        const r = {
                            name: v.name,
                            type: v.type,
            ...
            const structure = yield expandRecursively(vars.body.variables[11].variablesReference);
            (0, chai_1.expect)(structure).to.deep.equal(JSON.parse(fs.readFileSync(path.join(utils_1.testProgramsDir, '..', 'var_nest_expected.json'), { encoding: 'utf-8' })));
        });
    });
});

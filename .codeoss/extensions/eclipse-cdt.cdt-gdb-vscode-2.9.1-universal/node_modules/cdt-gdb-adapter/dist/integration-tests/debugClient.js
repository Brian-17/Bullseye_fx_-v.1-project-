...
class CdtDebugClient extends debugadapter_testsupport_1.DebugClient {
     ...
     */
    waitForOutputEvent(category_1, output_1) {
        return __awaiter(this, arguments, void 0, function* (category, output, outputIsRegex = false, timeout = this.defaultTimeout) {
            ...
            return new Promise((resolve, reject) => {
                ...
                const timer = setTimeout(() => {
                    ...
                    reject(new Error(`no output event with category '${category}' ${output === undefined
                        ? ''
                        : `and output message '${output}' ${outputIsRegex ? '(regex)' : ''}`} received after ${timeout} ms`));
                }, timeout);
     ...
     */
    getDebugConsoleOutput(launchArgs_1, category_1, output_1) {
        return __awaiter(this, arguments, void 0, function* (launchArgs, category, output, outputIsRegex = false) {
            const outputEvent = this.waitForOutputEvent(category, output, outputIsRegex);
            yield Promise.all([
                this.waitForEvent('initialized'),

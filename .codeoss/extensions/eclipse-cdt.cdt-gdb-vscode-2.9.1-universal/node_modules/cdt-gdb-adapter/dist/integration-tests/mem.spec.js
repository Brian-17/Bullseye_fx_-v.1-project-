...
describe('Memory Test Suite', function () {
    ...
    it('can write memory and handle memory notifications from gdb', function () {
        return __awaiter(this, void 0, void 0, function* () {
            const addrOfArray = parseInt((yield dc.evaluateRequest({
                expression: '&array[0]',
                frameId: frame.id,
            ...
            yield dc.evaluateRequest({
                expression: '>set array[0] = 0xde',
                frameId: frame.id,
                context: 'repl',
            });
            const output = yield memoryEvent;
            (0, chai_1.expect)(parseInt(output.body.memoryReference)).eq(addrOfArray);
            (0, chai_1.expect)(output.body.count).eq(1);
        });
    });

...
/**
 * SessionTimeServlet API helpers — direct (pure-TS) port of the React
 * `sessionTimeService.ts`. Handles communication with the session validation
 * endpoint, strips the CSRF prefix, and subtracts the latency buffer. Returns
 * `undefined` on any failure so the session-timeout feature never crashes the
 * running application.
 */

import { SESSION_CONFIG } from "./session-timeout.config";
...
/**
 * Parse the servlet response text into a SessionResponse object. Handles the
 * CSRF protection prefix.
 *
 * @param text - Raw response text from servlet.
 * @returns Parsed session response, or undefined if parsing fails.
 */
export function parseResponseResult(text: string): SessionResponse | undefined {
...
/**
 * Call SessionTimeServlet API. Internal function used by both poll and extend.
...
/**
 * Poll SessionTimeServlet to check remaining session time. Called periodically
...
/**
 * Extend the current session time. Called when the user clicks "Continue" in the
 * warning modal.
 */

...
/** Ready/error callbacks re-exported for consumers binding the component. */
export type { AgentforceErrorHandler, AgentforceReadyHandler };

/**
 * Inputs accepted by {@link AgentforceConversationClientComponent}. The Angular
 * port of the React `AgentforceConversationClientProps`; each field maps to a
 * component `input()` (with `agentId` required). Config is passed through to the
 * embed client as-is.
 */
